...
class DeConvLayer(nn.Module):
    def __init__(self, c_in, c_out=None):
        ...
        self.upConv = nn.ConvTranspose1d(
            in_channels=c_in, out_channels=c_out, kernel_size=3, stride=2, padding=2
        )
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None):
        ...
        x = x + self.dropout(self.cross_attention(x, cross, cross, attention_mask=cross_mask)[0])
        y = x = self.norm2(x)
...
class YformerDecoder(nn.Module):
    def __init__(self, attn_layers=None, conv_layers=None, norm_layer=None):
        ...
        self.first_attn = YformerDecoderLayer(
            AttentionLayer(FullAttention(False), d_model=512, n_heads=8), d_model=512, d_ff=2048
        )
        self.norm = norm_layer

    def forward(self, x_list, fut_x_list, attention_mask=None):
        # x [B, L, D]
        attns = []
        x = x_list.pop(0)
        fut_x = fut_x_list.pop(0)
        x = self.first_attn(x, fut_x, cross_mask=attention_mask)
        x = self.first_conv(x)  # upsample to connect with other layers from encoder
        if self.conv_layers is not None:
            if self.attn_layers is not None:
                for cross_x, cross_fut_x, attn_layer, conv_layer in zip(
                    x_list, fut_x_list, self.attn_layers, self.conv_layers
                ):
                    cross = torch.cat((cross_x, cross_fut_x), dim=1)
                    x = attn_layer(x, cross, cross_mask=attention_mask)
                    x = conv_layer(x)
            else:
