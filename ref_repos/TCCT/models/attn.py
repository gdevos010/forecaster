...
class ProbAttention(nn.Module):
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attention_mask):
        ...
        if self.mask_flag:
            attention_mask = ProbMask(B, H, L_Q, index, scores, device=V.device)
            scores.masked_fill_(attention_mask.mask, -np.inf)

        attention = torch.softmax(scores, dim=-1) # nn.Softmax(dim=-1)(scores)

        context_in[torch.arange(B)[:, None, None],
                   torch.arange(H)[None, :, None],
                   index, :] = torch.matmul(attention, V).type_as(context_in)
        if self.output_attention:
            attentions = (torch.ones([B, H, L_V, L_V])/L_V).type_as(attention).to(attention.device)
